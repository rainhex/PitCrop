...
CropBox::CropBox(qreal x, qreal y, qreal width, qreal height) :
    ...
    _rb(x + width - RBW(gPixWidth), y + height - RBW(gPixWidth), RBW(gPixWidth), RBW(gPixWidth), this)
{
    ...
    int w_mult = static_cast<int>((ceil(this->_width/base_width))-1);
    int h_mult = static_cast<int>((ceil(this->_height/base_height))-1);
...
void CropBox::resize(qreal new_w, qreal new_h){
...
void CropBox::increaseSize(){
    ...
    qreal new_w = base_width;
    qreal new_h = base_height;
    for(int i = 0; i < s; i++)
