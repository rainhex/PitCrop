...
void ZGraphicsView::mousePressEvent(QMouseEvent *event){
    if(event->buttons() & Qt::RightButton){
        ...
        int x = static_cast<int>(relativeOrigin.x() - (base_width/2));
        int y = static_cast<int>(relativeOrigin.y() - (base_height/2));
        gWindow->spawnCropbox(x, y);
    }
...
void ZGraphicsView::mouseReleaseEvent(QMouseEvent *e){
    QGraphicsView::mouseReleaseEvent(e);
}

void ZGraphicsView::mouseMoveEvent(QMouseEvent *e){
